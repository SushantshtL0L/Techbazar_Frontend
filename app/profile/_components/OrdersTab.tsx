...
export default function OrdersTab({ isDark = false }: OrdersTabProps) {
  ...
  if (orders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-neutral-50/50 dark:bg-neutral-900/50 rounded-[40px] border border-dashed border-neutral-200 dark:border-neutral-800">
        <FiPackage className="text-7xl text-neutral-300 dark:text-neutral-700 mb-6" />
        <h3 className="text-2xl font-bold text-neutral-600 dark:text-neutral-400">No orders yet</h3>
        <p className="text-neutral-400 dark:text-neutral-500 mt-2">Your shopping journey starts here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {orders.map((order) => (
        <div
          key={order._id}
          className={`rounded-[40px] overflow-hidden transition-all border shadow-sm ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-100 hover:border-neutral-200'}`}
        >
          {/* Order Header */}
          <div
          ...
          >
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 bg-teal-500/10 rounded-2xl flex items-center justify-center shadow-inner">
              ...
              <div>
                ...
                <p className={`text-lg font-bold ${isDark ? 'text-white' : 'text-neutral-900'}`}>#{order._id.slice(-8).toUpperCase()}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-12">
              ...
              <div className="hidden sm:block">
                ...
                <p className={`text-xl font-black ${isDark ? 'text-white' : 'text-neutral-900'}`}>Rs {order.totalAmount.toLocaleString()}</p>
              </div>
              ...
              <button className={`p-2 rounded-full ${isDark ? 'hover:bg-neutral-800' : 'hover:bg-neutral-100'} transition-colors`}>
          ...
          <AnimatePresence>
            {expandedOrder === order._id && (
              <motion.div
                ...
                className={`transition-colors ${isDark ? 'bg-black/20' : 'bg-neutral-50'}`}
              >
                <div className="p-8 border-t border-neutral-100 dark:border-neutral-800">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-6">
                      ...
                      {order.items.map((item: any, idx: number) => (
                        <div key={idx} className="flex items-center gap-4 group">
                          <div className={`w-20 h-20 rounded-2xl overflow-hidden p-2 flex items-center justify-center border ${isDark ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-neutral-100'}`}>
                            <img
                          ...
                          <div>
                            <h5 className={`font-bold ${isDark ? 'text-white' : 'text-neutral-900'}`}>{item.name}</h5>
                    ...
                    {/* Summary & Payment */}
                    <div className="bg-white dark:bg-neutral-800/30 rounded-3xl p-8 h-fit border border-neutral-100 dark:border-neutral-800 shadow-sm">
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center pb-4 border-b border-dashed border-neutral-200 dark:border-neutral-700">
                          ...
                          <span className={`font-black uppercase tracking-widest text-xs ${isDark ? 'text-white' : 'text-neutral-900'}`}>{order.paymentMethod}</span>
                        </div>
                        <div className="flex justify-between items-start pb-4 border-b border-dashed border-neutral-200 dark:border-neutral-700">
                          ...
                          <span className={`text-xs font-bold text-right max-w-[200px] ${isDark ? 'text-neutral-300' : 'text-neutral-600'}`}>
                        ...
                        <div className="flex justify-between items-center text-xl pt-2">
                          <span className={`font-bold ${isDark ? 'text-white' : 'text-neutral-900'}`}>Total Amount</span>
      ...
      {pagination && pagination.pages > 1 && (
        <div className={`pt-10 flex items-center justify-between border-t ${isDark ? 'border-neutral-800' : 'border-neutral-100'}`}>
          <p className="text-sm text-neutral-500 font-medium">
            Showing page <span className={`${isDark ? 'text-white' : 'text-neutral-900'} font-bold`}>{pagination.page}</span> of <span className={`${isDark ? 'text-white' : 'text-neutral-900'} font-bold`}>{pagination.pages}</span>
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => setPage(p => Math.max(1, p - 1))}
              disabled={pagination.page === 1}
              className={`px-6 py-3 rounded-2xl text-sm font-bold border transition-all flex items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed ${isDark ? 'bg-neutral-800 text-white border-neutral-700 hover:bg-neutral-700' : 'bg-white text-neutral-900 border-neutral-200 hover:bg-neutral-50'}`}
            >
              Previous
            </button>
            <button
